...
- **Shield proofs are intent-bound and domain-bound:** every prover-rewritable field — auth_domain, value, fees, recipient and producer commitments, memo ciphertext hashes — is folded into `deposit_id` via `H_intent`. Cross-deployment replay fails because `auth_domain` is in the intent, and prover-side redirection fails because the kernel recomputes the intent from request fields and rejects a mismatch.
- **Bridge deposits allocate per-deposit slots, not a shared balance:** every L1 ticket addressed to `deposit:<hex(intent)>` allocates a fresh slot keyed by a kernel-controlled monotonic `slot_id`. The slot's *content* is `(intent, amount)`, but the *key* is the opaque `slot_id` — depositors cannot influence, predict, or collide slot ids, and two L1 tickets with identical `(intent, amount)` content still get distinct ids. Shielding consumes a specific slot by id (the wallet observes its own slot id from the kernel state by-intent index after the deposit lands). Without per-deposit slots, an attacker observing a victim's L1 deposit could cheaply dust the same `deposit:<hex(intent)>` recipient string, push the bucket above the bound debit, and permanently brick the legitimate shield (which requires `bal == debit` exactly). Slots prevent this: dust deposits create their own orphan slots and cannot corrupt any other slot. Even an attacker who mirrors the legitimate debit exactly cannot brick the shield — whichever matching slot the wallet picks, shield succeeds and the user receives their note; at worst one slot is orphaned at the cost of a full debit to the attacker.
