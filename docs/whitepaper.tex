...
\(fee\) is burned. The deposit receipt itself is not directly withdrawable;
under the intent-bound scheme, \(deposit\_id\) is a hash of a witness only the
sender knows, so a third party who observes the L1 deposit cannot construct an
alternative shield against the same \(deposit\_id\).
