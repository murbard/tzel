...
with \(v_3 > 0\) for the producer-fee note. The circuit also enforces pairwise
distinct nullifiers within the transaction, while the rollup checks each
published nullifier against the global nullifier set.
...
\begin{itemize}
  ...
  \item global freshness of every published nullifier;
...
\begin{itemize}
  ...
  \item \textbf{Double-spend resistance.} Nullifiers are unique per
        note-position pair, pairwise distinct within a transaction, and checked
        against a global on-chain set.
